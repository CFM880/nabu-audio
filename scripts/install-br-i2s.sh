...
# Update only the audio1 machine driver, preserving a verified original.
set -euo pipefail
repo=$(cd -- "$(dirname -- "$0")/.." && pwd)
release=6.14.11-nabu-audio1
relative=kernel/sound/soc/qcom/snd-soc-sm8150.ko
...
backup=/var/lib/nabu-audio/br-i2s/$release
mode=${1:---install}
...
test -f "$target"
test ! -L "$target"
test "$(modinfo -n snd_soc_sm8150)" = "$target"
...
if [ "$mode" = --rollback ]; then
    ...
    replacement=$work/snd-soc-sm8150.ko
    test "$(modinfo -F name "$replacement")" = snd_soc_sm8150
...
staging=$(mktemp "$(dirname "$target")/.br-i2s.XXXXXX")
